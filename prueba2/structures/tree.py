...
class Tree:
    ...
    def __init__(self, root: Node | NodeA | None = None) -> None:
        self.root: Node | NodeA | None = root
    ...
    def _print_tree(
        ...
        children: list[Node | NodeA] = root.children or []
    ...
    def print_limited(
        self,
        optimal_path: list[Node | NodeA] | None = None,
        goals: list[Node | NodeA] | None = None,
        max_depth: int = 4,
    ) -> None:
        if self.root is None:
            return
    ...
    def _print_limited(
        self,
        root: Node | NodeA,
        optimal_path: list[Node] | list[NodeA],
        goals: list[Node],
        prefix: str,
        is_last: bool,
        max_depth: int,
        ...
        connector: str = "└── " if is_last else "├── "
        text: str = (
            root.data + f"  f={root.f} h={root.h})" + "  Nivel:" + str(root.depth)
        )
        ...
        visible: list[Node | NodeA] = [
            c for c in children if c in optimal_path or c in goals
        ]
