...
def generate_rules_for_class(
    df: pd.DataFrame, class_value: str
) -> list[PrismRule]:
    ...
    while count_examples(remaining, class_value) > 0:
        ...
        while True:
            ...
            working = working[matches(working, conditions)]
            ...
            if working[CLASS_COLUMN].eq(class_value).all():
                break
        ...
        remaining = remaining[~matches(remaining, conditions)]
...
def report_best_rules(df: pd.DataFrame, rules: list[PrismRule]) -> None:
    ...
    print(
        f"{GREEN}Mejores reglas con confianza >= {MIN_CONFIDENCE:.0%}:{RESET}"
    )
    print()
...
def best_condition(
    ...
    for column in df.columns:
        ...
        for value in df[column].unique():
            ...
            positives: int = int(
                (mask_value & (df[CLASS_COLUMN] == class_value)).sum()
            )
...
def format_rule_conditions(conditions: list[Condition], class_value: str) -> str:
    antecedent: str = " AND ".join(
